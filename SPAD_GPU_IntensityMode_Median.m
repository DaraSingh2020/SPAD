...
%% Dark image
darkImageDir='D:\LSCI_Image_Data\test_112021_13ns_DarkImage_gated';
darkImageN=0;
dHolder=double(imread(strcat(darkImageDir,'\image_',num2str(darkImageN),'.tiff')))*0;


parfor darkImageN=0:99
    darkImage_=double(imread(strcat(darkImageDir,'\image_',num2str(darkImageN),'.tiff')));
    dHolder=dHolder+darkImage_;
end

dHolder=dHolder/100;
threshold = prctile(dHolder,99.5,"all");
maskNormalPixels=dHolder>threshold;
imshow(maskNormalPixels)
...
parfor fileNumber=fileN_Start:fileN_End
    ...
    image(maskNormalPixels)=filteredImage(maskNormalPixels);
