...
Fre = 20e6;             % 20 MHZ
Duty = 0.5;             % Duty cycle 
GateL= 13.1;            % Gate length 
Nimage = 90;            % Number of Images REcorded 
...
Dir2='C:\SPAD\test\test_112021_13ns_DarkImage_gated\';
xx = 99; 
im=imread([Dir2,'image_',int2str(xx),'.tiff']); % dark image to find the hot spots 
dark1=double(im);
tre=11; % used to be 100 top 2.1% 
SizeDark1=(find(dark1>tre));
    ...
    numImage=0;
        for k=1:fileQuanty
            if rem(k,frame_no_period)==select_frame_no(ii)
                ...
                tempdata1 = tempdata;
                tempdata(find(dark1>tre))=nan; % hotspot correction  
